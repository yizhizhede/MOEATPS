...
// TSS 
static void TSS_analysis_correlation (Population_t* pop);		// correlation between variables and objectives
static void TSS_analysis_landscape (Population_t* pop);
static void TSS_analysis_conflict (Population_t* pop);
static void TSS_conflict_space (Population_t* pop);
static void TSS_satisfy_condition_LS (Population_t*pop, double *x);	// Conditions of landscape
static void TSS_satisfy_condition_CS (Population_t*pop, double *parent, double* child);	//  Conditions of conflict space 
static void TSS_optimize (Population_t* pop);

// Main framewok of MOEA/TSS
Population_t* moeatss (Problem_t *problem) {
...
static TSS_Pop_t* TSS_Pop_new (int popSize, int numVar, int numObj) {
	TSS_Pop_t* pop = NULL;

	pop = (TSS_Pop_t *)calloc (1, sizeof (TSS_Pop_t));
...
static void TSS_Pop_free (TSS_Pop_t* pop) {
	free (pop->obj);
	free (pop->vel);
	free (pop->var);
	free (pop);
...
static void TSS_init_pop (Population_t* pop, TSS_Pop_t* popOffspring) {
...
static void TSS_get_gamma (Matrix_t *M, double *gamma) {
	int 	rowDim = M->rowDim;
	int	colDim = M->colDim;
	double 	distance = 0;
	int 	i, j;
...
static void TSS_get_associate (Matrix_t* M, Matrix_t* V, int *associate) {
	int 	i, j, b;
	double	minDis, dis;
...
static Matrix_t* SBV_get_Var (TSS_Pop_t* popOffspring, int* byVar, int len) {
	double*		lowBound = Problem_getLowerBound ();
	double*		uppBound = Problem_getUpperBound ();
	int		popSize	 = popOffspring->popSize;
	int 		numVar   = popOffspring->numVar;
...
static void TSS_selection_by_var (TSS_Pop_t* popOffspring, Matrix_t *V, double theta, double* gammaV, int *byVar, int *sel, TSS_Pop_t* popSelectbyVar) {
	int		popSize	= popOffspring->popSize;
	int 		numVar  = popOffspring->numVar;
	int 		numObj 	= popOffspring->numObj;
	Matrix_t*	Var 	= NULL;
	...
	// associate
	TSS_get_associate (Var, V, associateV);
...
//
static Matrix_t* SBO_get_obj (TSS_Pop_t* popOffspring) {
	int 		popSize	= popOffspring->popSize;
	int		numObj  = popOffspring->numObj;
...
static void TSS_selection_by_obj (TSS_Pop_t* popOffspring, Matrix_t* Z, double theta, double* gammaZ, int* sel, TSS_Pop_t* popSelectbyObj) {
	int 		popSize	= popOffspring->popSize;
	int		numVar  = popOffspring->numVar;
	int		numObj  = popOffspring->numObj;
	Matrix_t*	Obj 	= NULL;
	...
	// associate
	TSS_get_associate (Obj, Z, associateZ);
...
//
static TSS_EP02_t* TSS_EP02_new (int EPSize, int numVar, int numObj) {
	TSS_EP02_t*	tssEP 	= NULL;
	int		i, j;

	// allocation	
	EPSize += 10;
	tssEP = (TSS_EP02_t*)calloc (1, sizeof (TSS_EP02_t));
	tssEP->var = (double*) calloc (EPSize*numVar+10, sizeof (double));
	tssEP->obj = (double*) calloc (EPSize*numObj+10, sizeof (double));
	tssEP->used = (int*) calloc (EPSize+10, sizeof (int));
	tssEP->idle = (int*) calloc (EPSize+10, sizeof (int));
	tssEP->uppNeighbor = (int*) calloc (EPSize*numObj+10, sizeof (int));
	tssEP->lowNeighbor = (int*) calloc (EPSize*numObj+10, sizeof (int));

	// initialization
	tssEP->len = 0;
	tssEP->top = 0;
	for (i=2; i<EPSize; tssEP->idle[tssEP->top++]=i, i++) {};
	for (j=0; j<numObj; j++) {
		tssEP->obj[0*numObj+j] = -1.0e+100;
		tssEP->obj[1*numObj+j] =  1.0e+100;
		tssEP->uppNeighbor[0*numObj+j] = 1;
		tssEP->lowNeighbor[0*numObj+j] = 0;
		tssEP->uppNeighbor[1*numObj+j] = 1;
		tssEP->lowNeighbor[1*numObj+j] = 0;
	}
	return tssEP;
}

//
static void TSS_EP02_add (Population_t* pop, TSS_EP02_t* tssEP, double *var, double* obj) {
	int	numVar	 = pop->var->colDim;
	int	numObj   = pop->obj->colDim;
	int	i, j, k, low;

	// pop one from idle
	i = tssEP->idle[tssEP->top-1];
	tssEP->top--;

	// put one into used
	tssEP->used[tssEP->len] = i;
	tssEP->len++;

	// copy to i
	memcpy (tssEP->var+i*numVar, var, numVar*sizeof (double));
	memcpy (tssEP->obj+i*numObj, obj, numObj*sizeof (double));

	// insert into link 
	for (j=0; j<numObj; j++) {
		k = tssEP->uppNeighbor[0*numObj+j];
		while (tssEP->obj[k*numObj+j] < obj[j]) {
			k = tssEP->uppNeighbor[k*numObj+j];
		}
		low = tssEP->lowNeighbor[k*numObj+j];
		tssEP->uppNeighbor[low*numObj+j] = i;
		tssEP->lowNeighbor[i*numObj+j] = low;
		tssEP->uppNeighbor[i*numObj+j] = k;
		tssEP->lowNeighbor[k*numObj+j] = i;
	}
}

//
static void TSS_EP02_delete (Population_t* pop, TSS_EP02_t* tssEP, int i) {
	int	numObj   = pop->obj->colDim;
	int	upp, low;
	int	j, a;

	// push i into idle
	tssEP->idle[tssEP->top] = i;
	tssEP->top++;

	// delete i from used
	for (a=0; a<tssEP->len; a++) if (i == tssEP->used[a]) {
		tssEP->used[a] = tssEP->used[tssEP->len-1];
		tssEP->len--;
		break;
	}
	...
	// delete link 
	for (j=0; j<numObj; j++) {
		upp = tssEP->uppNeighbor[i*numObj+j];
		low = tssEP->lowNeighbor[i*numObj+j];
		tssEP->lowNeighbor[upp*numObj+j] = low;
		tssEP->uppNeighbor[low*numObj+j] = upp;
	}
}
...
//
static void TSS_EP02_update (Population_t* pop, TSS_EP02_t* tssEP, double *var, double* obj) {
	int	numObj   = pop->obj->colDim;
	...
	// compare one with EP
	for (a=0; a<tssEP->len; a++) {
		i = tssEP->used[a];
		dominate = isDominate (tssEP->obj+i*numObj, obj, numObj);
	...
	for (a=0; a<tssEP->len; a++) {
		i = tssEP->used[a];
		dominate = isDominate (obj, tssEP->obj+i*numObj, numObj);
		if (1 == dominate) {
			TSS_EP02_delete (pop, tssEP, i);// delete one from EP
			a--;				// check 'a' again
		}
	}

	// add one to EP
	TSS_EP02_add (pop, tssEP, var, obj);	

	// if size is smaller
	if (tssEP->len <= Parameter_get()->popSize) return;
	...
	for (i=0; i<numObj; i++) {
		...
		for (a=0; a<tssEP->len; a++) {
			k = tssEP->used[a];
			for (j=0; j<numObj; j++) {
				buff[(j+i) % numObj] = tssEP->obj[k*numObj+j];
			}
	...
	// conpute crowdinD
	for (j=0; j<numObj; j++) {
		maxV = tssEP->obj[tssEP->lowNeighbor[1*numObj+j]*numObj+j];
		minV = tssEP->obj[tssEP->uppNeighbor[0*numObj+j]*numObj+j];
		if (maxV - minV < DBL_EPSILON) continue;
		i = tssEP->uppNeighbor[0*numObj+j];
		i = tssEP->uppNeighbor[i*numObj+j];
		while (1 != tssEP->uppNeighbor[i*numObj+j]) {
			upp = tssEP->uppNeighbor[i*numObj+j];
			low = tssEP->lowNeighbor[i*numObj+j];
			crowdingD[i] += (tssEP->obj[upp*numObj+j] - tssEP->obj[low*numObj+j]) / (maxV - minV); 
			i = tssEP->uppNeighbor[i*numObj+j];
		}
	}

	// find one with minCD
	for (a=0, b=-1; a<tssEP->len; a++) {
		i = tssEP->used[a];
		if (-1 == b) {
			minCD = crowdingD[i];
			b = i;
	...
	// delete one from EP
	if (-1 != b) {
		TSS_EP02_delete (pop, tssEP, b);	
	}
}

//
static void TSS_EP02_update (Population_t* pop, TSS_EP02_t* tssEP, TSS_Pop_t* popSelectbyVar, TSS_Pop_t* popSelectbyObj) {
	...
	for (i=0; i<popSize1; i++) {
		TSS_EP02_update (pop, tssEP, popSelectbyVar->var+i*numVar, popSelectbyVar->obj+i*numObj);
	}

	// update EP using popSelectbyObj
	for (i=0; i<popSize2; i++) {
		TSS_EP02_update (pop, tssEP, popSelectbyObj->var+i*numVar, popSelectbyObj->obj+i*numObj);
	}

	// copy EP to pop
	for (a=0; a<tssEP->len; a++) {
		i = tssEP->used[a];
		memcpy (pop->var->elements+a*numVar, tssEP->var+i*numVar, numVar*sizeof (double));
		memcpy (pop->obj->elements+a*numObj, tssEP->obj+i*numObj, numObj*sizeof (double));
	}
	
	// set size of pop
	pop->var->rowDim = tssEP->len;	
	pop->obj->rowDim = tssEP->len;	
}

typedef struct TSS_EP03_tag {
	Matrix_t*	Z;	//	
	double*		gammaZ;	//		
	double*		minObj;	//
	double*		L;	//
...
static TSS_EP03_t* TSS_EP03_new (Matrix_t* Z, double *gammaZ) {
	TSS_EP03_t*	tssEP = NULL;
	int		colDim = Z->colDim;
	int		i;

	// allocate
	tssEP = (TSS_EP03_t*)calloc (1, sizeof (TSS_EP03_t));
	tssEP->minObj = (double*)calloc (colDim+10, sizeof (double));
	tssEP->L      = (double*)calloc (colDim+10, sizeof (double));

	// init
	tssEP->Z = Z;
	tssEP->gammaZ = gammaZ;
	for (i=0; i<colDim; i++) {tssEP->L[i] = 1.0;}

	return tssEP;
}

static Matrix_t* EP03_get_obj (TSS_Pop_t* popAll, TSS_EP03_t* tssEP) {
	...
	// update minObj
	for (j=0; j<numObj; j++) {
		tssEP->minObj[j] = popAll->obj[j];
		for (i=1; i<popSize; i++) if (popAll->obj[i*numObj+j] < tssEP->minObj[j]) {
	 		tssEP->minObj[j] = popAll->obj[i*numObj+j];
		}
	}

	//
	for (j=0; j<numObj; j++) {
		if (tssEP->L[j] > DBL_EPSILON) for (i=0; i<popSize; i++) {
			Obj->elements[i*numObj+j] = (popAll->obj[i*numObj+j] - tssEP->minObj[j]) / tssEP->L[j];
		} else for (i=0; i<popSize; i++) {
			Obj->elements[i*numObj+j] = popAll->obj[i*numObj+j] - tssEP->minObj[j];
		}
	}
...
static void TEU_update_L (Population_t* pop, TSS_EP03_t* tssEP) {
	...
	for (j=0; j<numObj; j++) {
		tssEP->L[j] =  (int)(L[j]/minL);
	}
}

//
static void TSS_EP03_update (Population_t* pop, TSS_EP03_t* tssEP, TSS_Pop_t* popSelectbyVar, TSS_Pop_t* popSelectbyObj) {
	...
	int		numObj	 = pop->obj->colDim;
	Matrix_t*	Obj 	 = NULL;
	TSS_Pop_t*	popAll 	 = TSS_Pop_new (popSize0+popSize1+popSize2, numVar, numObj);
	Matrix_t*	Z 	 = tssEP->Z;
	double*		gammaZ 	 = tssEP->gammaZ;
...
static  Matrix_t* TSS_get_RV (int rowDim, int colDim) {
...
// reproduce using CSO
static void TSS_reproduce_CSO (Population_t* pop, TSS_Pop_t* popParent, TSS_Pop_t* popOffspring) {
	double*		lowBound = Problem_getLowerBound ();
	double*		uppBound = Problem_getUpperBound ();
	...
	while (n > 0) {
		...
		TSS_satisfy_condition_CS (pop, popParent->var+loser*numVar, popOffspring->var+k*numVar);
...
// TSS reproduce
static void TSS_reproduce (Population_t* pop, TSS_Pop_t* popSelectbyVar, TSS_Pop_t* popSelectbyObj, TSS_Pop_t* popOffspring){
	int 	popSize1= popSelectbyVar->popSize;
	int 	popSize2= popSelectbyObj->popSize;
	...
	// popParent
	TSS_Pop_t* popParent = TSS_Pop_new (popSize, numVar, numObj);
	...
	// CSO
	TSS_reproduce_CSO (pop, popParent, popOffspring); 

	// free popParent
	TSS_Pop_free (popParent);
}
...
static void TSS_optimize (Population_t* pop) {
	...
	int		numObj  = pop->obj->colDim;
	int		numVar  = pop->var->colDim;
	TSS_Pop_t*	popSelectbyVar 	= TSS_Pop_new (Z->rowDim, numVar, numObj);
	TSS_Pop_t*	popSelectbyObj 	= TSS_Pop_new (Z->rowDim, numVar, numObj);
	TSS_Pop_t*	popOffspring 	= TSS_Pop_new (4*Z->rowDim+10, numVar, numObj);
	TSS_EP02_t*	tssEP02		= NULL;
	TSS_EP03_t*	tssEP03		= NULL;
	int		EPSize	 	= Z->rowDim;
	int		byVar[numObj+10];
	...
	// if numObj = 2
	if (2 == numObj) { 
		tssEP02 = TSS_EP02_new (EPSize, numVar, numObj);
	} else { 
		tssEP03 =  TSS_EP03_new (Z, gammaZ);
	}
	...
	// get V
	V = TSS_get_RV (Z->rowDim, k);			

	// set popOffspring size
	popOffspring->popSize = V->rowDim + Z->rowDim;

	// init popOffspring
	TSS_init_pop (pop, popOffspring);

	// get gammaV and gammaZ
	TSS_get_gamma (V, gammaV);
	TSS_get_gamma (Z, gammaZ);

	// set selection flag
	memset (sel, 0, popOffspring->popSize*sizeof (int));

	// selection
	TSS_selection_by_var (popOffspring, V, TSS_get_theta(), gammaV, byVar, sel, popSelectbyVar); 
	TSS_selection_by_obj (popOffspring, Z, TSS_get_theta(), gammaZ, sel, popSelectbyObj);

	// update external population
	if (2 == numObj) {
		TSS_EP02_update (pop, tssEP02, popSelectbyVar, popSelectbyObj); 
	} else {
		TSS_EP03_update (pop, tssEP03, popSelectbyVar, popSelectbyObj);
	}
	...
	while (0 == isTerminal (pop)) {
		...
		// reproduce
		TSS_reproduce (pop, popSelectbyVar, popSelectbyObj, popOffspring);
...
static void SLPSO_optimize (Population_t* pop, double (*g_func)(double *f, double *w, int M), double *gW) {
	...
	while (t < maxFEs) {
		...
		for (i=0; i<m; i++) {
			...
			TSS_satisfy_condition_LS (pop, X+i*numVar);		
...
static void TSS_analysis_correlation (Population_t* pop) {
	double*		lowBound = Problem_getLowerBound ();
	double*		uppBound = Problem_getUpperBound ();
	int		numVar = pop->var->colDim;
	int		numObj = pop->obj->colDim;
...
//
static void TSS_satisfy_condition_LS (Population_t*pop, double *x) {
	double*	lowBound = Problem_getLowerBound ();
	double*	uppBound = Problem_getUpperBound ();
	int	numVar  = pop->var->colDim;
	int 	i, j;
...
static void TSS_satisfy_condition_CS (Population_t*pop, double *parent, double* child) {
	int	numVar  = pop->var->colDim;
	int	i, r = rand () % 100;
	double	d;
